...
class SweepNet1DDet(torch.nn.Module):
    ...
    def __init__(self, H, W, outputs, channels):
        ...
        self.conv4 = torch.nn.Conv1d(conv3_channels, conv4_channels, kernel_size=conv4_kernel, stride=conv4_stride, padding=3)
        self.relu4 = torch.nn.ReLU()
        
        self.conv5 = torch.nn.Conv1d(conv4_channels, conv5_channels, kernel_size=conv5_kernel, stride=conv5_stride, padding=3)
        self.relu5 = torch.nn.ReLU()
        
        self.conv6 = torch.nn.Conv1d(conv5_channels, conv6_channels, kernel_size=conv6_kernel, stride=conv6_stride, padding=3)
        self.relu6 = torch.nn.ReLU()
        ...
        out_conv3 = self.compute_out_shape(out_pool2, (conv3_kernel, conv3_kernel), (conv3_stride, conv3_stride))
        out_pool3 = self.compute_out_shape(out_conv3, (pool3_kernel, pool3_kernel), (pool3_stride, pool3_stride))
        out_conv4 = self.compute_out_shape((out_pool3[0]+6, out_pool3[1]+6), (conv4_kernel, conv4_kernel), (conv4_stride, conv4_stride))
        out_conv5 = self.compute_out_shape((out_conv4[0]+6, out_conv4[1]+6), (conv5_kernel, conv5_kernel), (conv5_stride, conv5_stride))
        out_conv6 = self.compute_out_shape((out_conv5[0]+6, out_conv5[1]+6), (conv5_kernel, conv5_kernel), (conv5_stride, conv5_stride))
        
        # init output layer
        self.fc = torch.nn.Linear(out_conv6[1]*32, 2)
